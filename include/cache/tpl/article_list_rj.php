...
<div class="head">
    <div class="wrap w980">
        ...
        <div class="top_wz"><a href="#"onclick="this.style.behavior='url(#default#homepage)';this.setHomePage('http://localhost');">设为首页</a>| <a href="#" onClick="window.external.AddFavorite('http://localhost','本地网站')">加入收藏</a> | <a href="/index.php?c=article&a=type&tid=70">联系我们</a></div>
          <ul>
          <li><a href="/">首页</a></li>
  ...
  <div class="wrap w980">
    ...
    <div class="clearfix">
      <div class="left_cont">
        ...
        <?php $vn=0;$tablev=syClass("syModel")->findSql("select tid,molds,pid,classname,gourl,litpic,title,keywords,description,orders,mrank,htmldir,htmlfile,mshow from dy_classtype where  mshow='1' and pid='69'  order by orders desc,tid limit 11");foreach($tablev as $v){ $v["tid_leafid"]=$sy_class_type->leafid($v["tid"]);$v["n"]=$vn=$vn+1; $v["classname"]=stripslashes($v["classname"]);$v["description"]=stripslashes($v["description"]); $v["url"]=html_url("classtype",$v); ?>
        ...
        <div class="about">
          <h2><?php echo $type['classname'] ?></h2>
               <ul class="news_list">
                 ...
                 <li><span<?php if(newest($v['addtime'],24)){ ?> style="color:red"<?php } ?>>[<?php echo date('Y-m-d',$v['addtime']) ?>]</span><a href="<?php echo $v['url'] ?>" style="<?php echo $v['style'] ?>" target="_blank"><b>·</b><?php echo $v['title'] ?></a></li>
                 <?php } ?>
                 </ul>
